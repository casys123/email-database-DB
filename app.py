...
# =============================
# MMF LeadHarvester
# Streamlit app to find General Contractors, Builders, and Architects in
# Miami-Dade & Broward County, extract emails, and export a CSV.
#
# FIRST RUN NOTES (no API keys required):
# - Create a requirements.txt with the following lines:
#     streamlit
#     requests
#     beautifulsoup4
#     pandas
#     tldextract
# - Deploy: push app.py + requirements.txt to GitHub, then launch on Streamlit Cloud.
# - This app uses DuckDuckGo HTML results (no key) + light site crawling (contact/about pages).
# - Be respectful of websites. Use small limits & delays. Results vary; refine queries as needed.
# =============================
...
def duckduckgo_search(query: str, max_results: int = 20, pause: float = 1.5):
    ...
    Returns a list of dicts with title and href.
    """
    results = []
    params = {"q": query}
...
def is_relevant_to_counties(text: str) -> str:
    """Return a best-guess county label if keywords appear, else ''."""
    t = (text or "").lower()
...
if run_button:
    ...
    if seed_file is not None:
        try:
            ...
            seed_sites = [u if u.startswith("http") else f"https://{u}" for u in seed_df.get("website", []) if pd.notna(u)]
    ...
    for idx, site in enumerate(unique_sites, start=1):
        label = "Unknown"
        # naive category guess by domain hints
...
- Send small, thoughtful outreach sequences (2–3 messages) with clear value: fast turnarounds, rental-unit refresh expertise, competitive pricing, and photos/case studies.
- Always include an unsubscribe option to respect CAN-SPAM.
- Consider validating emails with a checker before sending to reduce bounces.
    """
)

st.caption("© Miami Master Flooring · MMF LeadHarvester · For research and outreach to publicly listed contacts only.")
